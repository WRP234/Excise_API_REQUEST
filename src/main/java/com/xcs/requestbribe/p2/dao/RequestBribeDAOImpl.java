    ...
    public RequestBribe RequestBribegetByCon(RequestBribegetByConReq req) {
        ...
        StringBuilder sqlBuilder = new StringBuilder()
                .append(" SELECT DISTINCT " +
                		" OPS_REQUEST_BRIBE.* " +
                		" FROM OPS_REQUEST_BRIBE WHERE OPS_REQUEST_BRIBE.IS_ACTIVE = 1 ");
...
//                item.setOPERATION_DEPT_CODE(rs.getString("OPERATION_DEPT_CODE"));
        ...
        sqlBuilder = new StringBuilder()
                .append(" SELECT DISTINCT OPS_REQUEST_BRIBE_DETAIL.* "
                		+ "FROM OPS_REQUEST_BRIBE "
//                		+ "LEFT JOIN OPS_REQUEST_BRIBE ON OPS_REQUEST_BRIBE_REWARD.BRIBE_REWARD_ID = OPS_REQUEST_BRIBE.BRIBE_REWARD_ID "
                		+ "LEFT JOIN OPS_REQUEST_BRIBE_DETAIL ON OPS_REQUEST_BRIBE.BRIBE_ID = OPS_REQUEST_BRIBE_DETAIL.BRIBE_ID ");
        ...
        sqlBuilder = new StringBuilder() 
        		.append(" SELECT DISTINCT "
        				+ "OPS_REQUEST_BRIBE_REWARD.* "
        				+ "FROM OPS_REQUEST_BRIBE "
        				+ "LEFT JOIN OPS_REQUEST_BRIBE_REWARD ON OPS_REQUEST_BRIBE.BRIBE_REWARD_ID = OPS_REQUEST_BRIBE_REWARD.BRIBE_REWARD_ID ");
        if (req.getBRIBE_ID() != null && !"".equals(req.getBRIBE_ID())) {
        ...
        @SuppressWarnings("unchecked")
        List<RequestBribeReward> requestBribeRewards = getJdbcTemplate().query(sqlBuilder.toString(), new RowMapper<RequestBribeReward>() {
        	
        	public RequestBribeReward mapRow(ResultSet rs, int rowNum) throws SQLException {
        		RequestBribeReward item = new RequestBribeReward();
        		
        		item.setBRIBE_REWARD_ID(rs.getString("BRIBE_REWARD_ID"));
        		item.setINDICTMENT_ID(rs.getString("INDICTMENT_ID"));
        		item.setHAVE_NOTICE(rs.getString("HAVE_NOTICE"));
        		item.setIS_ACTIVE(rs.getString("IS_ACTIVE"));
        ...
        data.getRequestBribeRewards().addAll(requestBribeRewards);
        

        sqlBuilder = new StringBuilder()
                .append(" SELECT DISTINCT "
                		+ "OPS_REQUEST_BRIBE_STAFF.* "
                		+ "FROM OPS_REQUEST_BRIBE "
//                		+ "LEFT JOIN OPS_REQUEST_BRIBE ON OPS_REQUEST_BRIBE_REWARD.BRIBE_REWARD_ID = OPS_REQUEST_BRIBE.BRIBE_REWARD_ID "
                		+ "LEFT JOIN OPS_REQUEST_BRIBE_STAFF ON OPS_REQUEST_BRIBE.BRIBE_ID = OPS_REQUEST_BRIBE_STAFF.BRIBE_ID ");
